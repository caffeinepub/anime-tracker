...
import { useUpdateWatchlistEntry, useUpdateTotalEpisodes } from '../hooks/useQueries';
...
export default function WatchlistEntryDialog({ entry, watchlistName, open, onOpenChange }: WatchlistEntryDialogProps) {
  ...
  const handleEpisodesSave = async () => {
    ...
    if (episodes > Number(entry.anime.episodesAvailable)) {
      toast.error(`Episodes watched cannot exceed ${entry.anime.episodesAvailable}`);
      return;
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[3412px] w-[90vw] max-h-[90vh] overflow-y-auto bg-[#3a3a3a]">
        ...
        <div className="flex flex-col items-center space-y-6">
          ...
          {/* Title */}
          <h2 className="text-3xl font-bold text-center">{entry.anime.title}</h2>
          ...
          <div className="grid grid-cols-4 gap-3 w-full max-w-4xl">
            {entry.anime.genres.map((genre, idx) => (
              <Badge key={idx} className={`${getGenreColor(genre)} justify-center text-base py-2`}>
                {genre}
              </Badge>
            ))}
          </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-base text-muted-foreground">
                  <span className="font-medium">Status:</span> {entry.anime.airingStatus}
                </p>
                <p className="text-base text-muted-foreground">
                  <span className="font-medium">Public Rating:</span> {Number(entry.anime.publicRating)}/100
                </p>
                <p className="text-base text-muted-foreground">
                  <span className="font-medium">Aired:</span> {entry.anime.airedDate}
                </p>
              </div>
              <div>
                <p className="text-base text-muted-foreground">
                  <span className="font-medium">Your Rating:</span> {entry.personalRating > 0 ? `${entry.personalRating}/10` : 'N/A'}
                </p>
