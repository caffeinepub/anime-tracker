...
import { useUpdateGenres, useGetGlobalGenres } from '../hooks/useQueries';
...
export default function GenreEditor({ entry, watchlistName, open, onOpenChange }: GenreEditorProps) {
  const [currentGenres, setCurrentGenres] = useState<string[]>([]);
  const [newGenre, setNewGenre] = useState('');
  const { data: globalGenres, isLoading: globalGenresLoading } = useGetGlobalGenres();
      ...
      <DialogContent className="bg-black border-4 border-yellow-500 text-white max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            <h3 className="text-sm font-medium text-yellow-400">Current Genres</h3>
          ...
          {/* Add New Genre */}
          <div className="space-y-2">
            <h3 className="text-sm font-medium text-yellow-400">Add New Genre</h3>
          ...
          {!globalGenresLoading && globalGenres && globalGenres.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-sm font-medium text-yellow-400">Quick Add from Your Watchlists</h3>
