...
export default function SystemInfoPanel() {
  ...
  const [currentCanisterId, setCurrentCanisterId] = useState<string>('Loading...');
      ...
      try {
        ...
        try {
          ...
          if (response.ok) {
            ...
            const canisterId = envData.BACKEND_CANISTER_ID || envData.backend_canister_id;
            if (canisterId) {
              setCurrentCanisterId(canisterId);
              return;
            }
  ...
  useEffect(() => {
    const calculateSizes = async () => {
      try {
        ...
        const limitInBytes = ramUsageLimitUnit === 'GB' 
  ...
  return (
    <div className="bg-black rounded-lg border-4 border-yellow-500 mb-6 overflow-hidden">
      ...
      >
        <h2 className="text-lg font-bold text-yellow-500">System Information</h2>
        {showPanel ? (
      ...
      {showPanel && (
        <div className="p-4 space-y-4 border-t-2 border-yellow-500">
          ...
          {showMetadata && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 animate-in fade-in duration-300">
              <div className="space-y-1">
                <Label className="text-white text-xs font-medium">Total Metadata Storage</Label>
                <div className="bg-gray-900 rounded-lg p-2 border-2 border-yellow-600">
                  <p className="text-base font-bold text-yellow-400">{metadataSize}</p>
                  <p className="text-[10px] text-gray-400 mt-0.5">IndexedDB data excluding images</p>
                </div>
              </div>

              <div className="space-y-1">
                <Label className="text-white text-xs font-medium">Total Image Storage</Label>
                <div className="bg-gray-900 rounded-lg p-2 border-2 border-yellow-600">
                  <p className="text-base font-bold text-yellow-400">{imageSize}</p>
                  <p className="text-[10px] text-gray-400 mt-0.5">All cached image blobs</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            <Label className="text-white text-xs font-medium">RAM Limit (GB)</Label>
          ...
          <div className="space-y-2">
            <Label className="text-white text-xs font-medium">RAM Usage Limit</Label>
            <div className="flex gap-2 items-end">
              ...
              <div className="flex gap-1">
                <Button
                  ...
                  className={ramUsageLimitUnit === 'MB' ? 'bg-yellow-500 text-black hover:bg-yellow-600' : 'border-yellow-500 text-yellow-500'}
                >
                  MB
                </Button>
                <Button
                  ...
                  className={ramUsageLimitUnit === 'GB' ? 'bg-yellow-500 text-black hover:bg-yellow-600' : 'border-yellow-500 text-yellow-500'}
                >
                  GB
                </Button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              <div className="bg-gray-900 rounded-lg p-2 border-2 border-yellow-600">
                ...
                <p className="text-sm font-bold text-yellow-400">{currentRamUsage}</p>
              </div>
              <div className="bg-gray-900 rounded-lg p-2 border-2 border-yellow-600">
                <p className="text-[10px] text-gray-400">Available</p>
                <p className="text-sm font-bold text-yellow-400">{availableRam}</p>
              </div>
            </div>
            <p className="text-[10px] text-gray-400">
              Set maximum memory usage threshold for the app with real-time monitoring
            </p>
          </div>

          {/* Backend Canister Configuration */}
          <div className="space-y-2">
            <Label className="text-white text-xs font-medium">Backend Canister ID</Label>
            <div className="bg-gray-900 rounded-lg p-2 border-2 border-yellow-600">
              <p className="text-xs font-mono text-yellow-400 break-all">{currentCanisterId}</p>
            </div>
            <p className="text-[10px] text-gray-400">
              Current backend canister identifier retrieved from application configuration
            </p>
          </div>
        </div>
      )}
