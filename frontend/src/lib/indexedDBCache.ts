...
class IndexedDBCache {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(IMAGE_STORE)) {
          const imageStore = db.createObjectStore(IMAGE_STORE, { keyPath: 'key' });
  ...
  private async getMetadata(): Promise<CacheMetadata> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        resolve(request.result || { totalSize: 0, entries: 0, lastCleanup: Date.now() });
      };
  ...
  async set<T>(key: string, data: T, ttl: number = DEFAULT_TTL): Promise<boolean> {
    try {
      await this.ensureInitialized();
  ...
  private async collectLeastUsedEntries(): Promise<Array<{ key: string; lastAccessed: number; size: number; accessCount: number }>> {
    await this.ensureInitialized();
    const entries: Array<{ key: string; lastAccessed: number; size: number; accessCount: number }> = [];
  ...
  async clear(): Promise<void> {
    try {
      await this.ensureInitialized();
      return new Promise((resolve, reject) => {
        const transaction = this.db!.transaction([STORE_NAME, IMAGE_STORE, METADATA_STORE], 'readwrite');
        
        const clearStore = transaction.objectStore(STORE_NAME).clear();
        const clearImages = transaction.objectStore(IMAGE_STORE).clear();
  ...
  async getStats(): Promise<{ size: number; entries: number; sizeFormatted: string }> {
    try {
      const metadata = await this.getMetadata();
      ...
      return {
        ...
        sizeFormatted: metadata.totalSize > 1024 * 1024 * 1024 ? `${sizeInGB} GB` : `${sizeInMB} MB`,
      };
    } catch (error) {
  ...
  async setImage(url: string, blob: Blob): Promise<boolean> {
    try {
      ...
      return new Promise((resolve) => {
        const transaction = this.db!.transaction([IMAGE_STORE, METADATA_STORE], 'readwrite');
        const store = transaction.objectStore(IMAGE_STORE);
