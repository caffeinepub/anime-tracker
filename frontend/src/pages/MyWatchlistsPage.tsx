...
import { useGetUserWatchlists, useDeleteWatchlist } from '../hooks/useQueries';
import { useActor } from '../hooks/useActor';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Plus, Trash2, X, AlertCircle, RefreshCw, ChevronDown, ChevronUp } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
type SortOption = 'none' | 'alphabetical' | 'rating' | 'episodes' | 'personalRating' | 'airedDate';
type SortDirection = 'asc' | 'desc';

export default function MyWatchlistsPage() {
  const { identity } = useInternetIdentity();
  const { actor, isFetching: actorFetching } = useActor();
  const { data: userWatchlistsData, isLoading: watchlistsLoading, isError: watchlistsError, error: watchlistsErrorDetails, refetch } = useGetUserWatchlists();
  ...
  const availableGenres = useMemo(() => {
    ...
    currentWatchlist.entries.forEach(entry => {
      entry.anime.genres.forEach(genre => genreSet.add(genre));
    });
  ...
  const filteredAndSortedEntries = useMemo(() => {
    ...
    if (titleSearch.trim()) {
      ...
      entries = entries.filter(entry => {
        const titleMatch = entry.anime.title.toLowerCase().includes(searchLower);
      ...
      entries = [...entries].sort((a, b) => {
        ...
        switch (sortBy) {
          case 'alphabetical':
            ...
            break;
          case 'rating':
            comparison = Number(a.anime.publicRating) - Number(b.anime.publicRating);
            break;
          case 'episodes':
    ...
    return entries;
  }, [currentWatchlist, selectedGenres, showBookmarkedOnly, titleSearch, notesSearch, synopsisSearch, sortBy, sortDirection]);
  ...
  const getSortLabel = (sortOption: SortOption): string => {
    switch (sortOption) {
      ...
      case 'personalRating': return 'Personal Rating';
      case 'rating': return 'Public Rating';
      case 'episodes': return 'Episodes Watched';
      case 'airedDate': return 'Aired Date';
      default: return '';
    }
  };
    ...
    return (
      <div className="container mx-auto px-4">
        <div className="text-center py-16">
          <h2 className="text-2xl font-bold mb-4">Please log in to view your watchlists</h2>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-center py-8">
            <div className="text-center space-y-4">
              ...
              <p className="text-muted-foreground">
                {actorFetching ? 'Connecting to backend...' : 'Loading watchlists...'}
              </p>
            </div>
          </div>
  ...
  return (
    <div className="w-full">
      <div className="container mx-auto px-4 space-y-6">
        ...
        {watchlists.length === 0 ? (
          ...
          <>
            ...
            <div className="fixed top-0 left-0 right-0 z-40 bg-background pt-4 pb-4" style={{ marginTop: '64px' }}>
              <div className="container mx-auto px-4">
                  ...
                  {isControlPanelCollapsed ? (
                    <div className="p-3 flex items-center justify-between">
                      <div className="flex items-center gap-4 flex-1">
                        <span className="text-sm font-medium text-yellow-500">
                          {selectedWatchlist ? `${selectedWatchlist} (${filteredAndSortedEntries.length} titles)` : 'Select a watchlist'}
                        </span>
                        {selectedWatchlist && (
                          <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            {showBookmarkedOnly && <span>• Bookmarked</span>}
                            {selectedGenres.length > 0 && <span>• {selectedGenres.length} genre{selectedGenres.length > 1 ? 's' : ''}</span>}
                            {sortBy !== 'none' && <span>• Sorted by {getSortLabel(sortBy)}</span>}
                          </div>
                        )}
                      </div>
                      <Button
                    ...
                    <div className="p-6 space-y-4">
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="text-lg font-semibold text-yellow-500">Watchlist Controls</h3>
                        <Button
                          variant="ghost"
                          size="sm"
                      ...
                      <div className="flex gap-4 flex-wrap items-end">
                        <div className="flex-1 min-w-[200px]">
                          <label className="text-sm font-medium mb-2 block">Select Watchlist</label>
                          <div className="flex gap-2">
                            <Select value={selectedWatchlist || ''} onValueChange={(value) => {
                              ...
                              <SelectTrigger className="flex-1 bg-black border-2 border-yellow-600 text-white">
                            ...
                            {selectedWatchlist && (
                              <Button
                                variant="destructive"
                                size="icon"
                                onClick={() => setWatchlistToDelete(selectedWatchlist)}
                        ...
                        {selectedWatchlist && (
                          <button
                            ...
                            className={`p-2 rounded transition-colors ${
                              showBookmarkedOnly ? 'bg-accent/20' : 'hover:bg-accent/10'
                            }`}
                            title={showBookmarkedOnly ? 'Show all titles' : 'Show bookmarked only'}
                          >
                            <img
                            ...
                            <PopoverContent className="w-80 max-h-96 overflow-y-auto bg-black border-4 border-yellow-500 text-white genre-filter-popup">
                              <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                  <h4 className="font-medium text-sm text-white">Filter by Genre</h4>
                        ...
                        {selectedWatchlist && (
                          <div className="flex gap-2">
                            <Select value={sortBy} onValueChange={(value: SortOption) => {
                              ...
                              <SelectContent className="bg-black border-2 border-yellow-600 text-white watchlist-dropdown-content">
                                <SelectItem value="none" className="text-white hover:bg-gray-800 focus:bg-gray-800 focus:text-white data-[highlighted]:bg-gray-800 data-[highlighted]:text-white cursor-pointer">
                                  No sorting
                                </SelectItem>
                                <SelectItem value="alphabetical" className="text-white hover:bg-gray-800 focus:bg-gray-800 focus:text-white data-[highlighted]:bg-gray-800 data-[highlighted]:text-white cursor-pointer">
                                  Alphabetical
                                </SelectItem>
                                <SelectItem value="personalRating" className="text-white hover:bg-gray-800 focus:bg-gray-800 focus:text-white data-[highlighted]:bg-gray-800 data-[highlighted]:text-white cursor-pointer">
                                  Personal Rating
                                </SelectItem>
                                <SelectItem value="rating" className="text-white hover:bg-gray-800 focus:bg-gray-800 focus:text-white data-[highlighted]:bg-gray-800 data-[highlighted]:text-white cursor-pointer">
                                  Public Rating
                                </SelectItem>
                                <SelectItem value="episodes" className="text-white hover:bg-gray-800 focus:bg-gray-800 focus:text-white data-[highlighted]:bg-gray-800 data-[highlighted]:text-white cursor-pointer">
                                  Episodes Watched
                                </SelectItem>
                                <SelectItem value="airedDate" className="text-white hover:bg-gray-800 focus:bg-gray-800 focus:text-white data-[highlighted]:bg-gray-800 data-[highlighted]:text-white cursor-pointer">
                                  Aired Date
                                </SelectItem>
                              </SelectContent>
                            </Select>
                            ...
                            {sortBy !== 'none' && (
                              <Select value={sortDirection} onValueChange={(value: SortDirection) => {
                                ...
                                <SelectContent className="bg-black border-2 border-yellow-600 text-white watchlist-dropdown-content">
                                  <SelectItem value="asc" className="text-white hover:bg-gray-800 focus:bg-gray-800 focus:text-white data-[highlighted]:bg-gray-800 data-[highlighted]:text-white cursor-pointer">
                                    Ascending
                                  </SelectItem>
                                  <SelectItem value="desc" className="text-white hover:bg-gray-800 focus:bg-gray-800 focus:text-white data-[highlighted]:bg-gray-800 data-[highlighted]:text-white cursor-pointer">
                                    Descending
                                  </SelectItem>
                                </SelectContent>
                              </Select>
                      ...
                      {selectedWatchlist && (
                        <div className="flex gap-4 flex-wrap">
                          ...
                          <div className="flex-1 min-w-[200px]">
                            <label className="text-sm font-medium mb-2 block">Search by Notes</label>
                            <Input
                              type="text"
                          ...
                          <div className="flex-1 min-w-[200px]">
                            <label className="text-sm font-medium mb-2 block">Search by Synopsis</label>
                      ...
                      {selectedWatchlist && (
                        <div className="text-sm text-muted-foreground">
                          Showing {filteredAndSortedEntries.length} of {currentWatchlist?.entries.length || 0} titles
                          {showBookmarkedOnly && ' (bookmarked only)'}
                          {sortBy !== 'none' && ` • Sorted by ${getSortLabel(sortBy)} (${sortDirection === 'asc' ? 'Ascending' : 'Descending'})`}
                        </div>
                      )}
                    </div>
                  )}
            ...
            <div style={{ height: isControlPanelCollapsed ? '60px' : '380px' }} />
          </>
        )}
      </div>
      ...
      <div className="container mx-auto px-4">
        <CreateWatchlistDialog open={showCreateDialog} onOpenChange={setShowCreateDialog} />

        {/* Delete Confirmation Dialog */}
        <AlertDialog open={!!watchlistToDelete} onOpenChange={(open) => !open && setWatchlistToDelete(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Delete Watchlist</AlertDialogTitle>
              <AlertDialogDescription>
                Are you sure you want to delete the watchlist "{watchlistToDelete}"? This action cannot be undone and will permanently remove all {currentWatchlist?.entries.length || 0} anime entries from this watchlist.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
