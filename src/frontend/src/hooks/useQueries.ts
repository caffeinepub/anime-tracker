...
export function useRemoveAnimeFromWatchlist() {
  ...
  return useMutation({
    mutationFn: async ({
      watchlistName,
      anilistId,
    }: { watchlistName: string; anilistId: bigint }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useUpdateWatchlistEntry() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("Actor not available");
      return actor.updateWatchlistEntry(
        watchlistName,
        anilistId,
        personalRating,
        episodesWatched,
...
export function useSetWatchlistPublic() {
  ...
  return useMutation({
    mutationFn: async ({
      watchlistName,
      isPublic,
    }: { watchlistName: string; isPublic: boolean }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useUpdateTotalEpisodes() {
  ...
  return useMutation({
    mutationFn: async ({
      anilistId,
      newTotalEpisodes,
    }: { anilistId: bigint; newTotalEpisodes: bigint }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useFetchAnime(
  ...
  return useQuery<AnimeResponse>({
    ...
    queryFn: async () => {
      ...
      const animeData: AnimeData[] = parsedResponse.data.Page.media.map(
        (anime: any) => ({
          id: anime.id,
          title: {
      ...
      // Sort using backend
      const sortedEntries = await actor.sortAnimeEntries(
        animeEntries,
        sortBy,
        sortOrder === "asc",
      );
      ...
      const sortedAnimeData = sortedEntries.map((entry: AnimeEntry) => {
        const originalAnime = animeData.find(
          (a) => a.id === Number(entry.anilistId),
        );
