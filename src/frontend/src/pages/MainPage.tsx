...
export default function MainPage() {
  ...
  const availableGenres = useMemo(() => {
    ...
    for (const anime of animeData.media) {
      if (anime.genres) {
        for (const genre of anime.genres) {
          genreSet.add(genre as string);
        }
      }
    }
  ...
  useEffect(() => {
    if (error && typeof error === "object" && "message" in error) {
      ...
      if (message.includes("RATE_LIMIT:")) {
        const waitTime = Number.parseInt(message.split("RATE_LIMIT: ")[1], 10);
  ...
  const handleClearCache = () => {
    ...
    toast.success("Cache cleared successfully", {
      description:
        "All cached anime data has been removed. Fresh data will be fetched from the API.",
    });
  };
  ...
  const animeInWatchlists = useMemo(() => {
    ...
    for (const watchlist of userWatchlistsData.watchlists) {
  ...
  const getErrorMessage = (
    ...
    if (message.includes("Rate limit exceeded")) {
      return {
        ...
        description:
          "You have exceeded the rate limit of 90 requests per minute. The request will be automatically retried shortly.",
        canRetry: true,
        isRateLimit: true,
      };
    }
    ...
    if (
      message.includes("PARSE_ERROR:") ||
      message.includes("Invalid response")
    ) {
      return {
        title: "API Response Error",
        description:
          "The Anilist API returned unexpected data. This is usually temporary and may be due to high traffic or rate limiting. The request will be automatically retried.",
        canRetry: true,
        isRateLimit: false,
      };
    }
    ...
    if (message.includes("MISSING_DATA:")) {
      return {
        ...
        description:
          "The API response did not contain anime data. This may be temporary - please try again.",
        canRetry: true,
        isRateLimit: false,
      };
    }
    ...
    <div className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <p className="text-xs text-muted-foreground mb-2">
                    Data is cached locally on your device using IndexedDB for
                    improved performance and offline access.
                  </p>
                  <Button
                    variant="destructive"
                    size="sm"
      ...
      {errorInfo && !errorInfo.isRateLimit && (
        <Alert variant="destructive">
          ...
          <AlertDescription className="space-y-2">
            ...
            {failureCount > 0 && (
              <p className="text-xs mt-2">Retry attempt {failureCount} of 3</p>
            )}
          </AlertDescription>
        </Alert>
      )}
      ...
      {isLoading && !isRefetching ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {Array.from(
            { length: itemsPerPage > 40 ? 40 : itemsPerPage },
            (_, i) => i,
          ).map((i) => (
            <Skeleton key={`skeleton-${i}`} className="h-[150px] rounded-lg" />
          ))}
        </div>
      ) : error && !isRefetching ? (
