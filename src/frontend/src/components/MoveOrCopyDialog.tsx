...
import {
  useCopyAnimeToWatchlist,
  useMoveAnimeToWatchlist,
...
export default function MoveOrCopyDialog({
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      toast.success(
        `Copied to ${selectedWatchlist} with all personal data preserved`,
      );
      onOpenChange(false);
    } catch (error: any) {
  ...
  const handleMove = async () => {
    ...
    try {
      ...
      toast.success(
        `Moved to ${selectedWatchlist} with all personal data preserved`,
      );
      onOpenChange(false);
    } catch (error: any) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[#3a3a3a]">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose a watchlist to move or copy "{entry.anime.title}" to. All
            personal data (rating, episodes watched, notes, and bookmark status)
            will be preserved.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          {availableWatchlists.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">
              No other watchlists available. This anime may already exist in all
              other watchlists.
            </p>
          ) : (
            <>
              <div className="space-y-2">
                <label
                  htmlFor="move-copy-watchlist-select"
