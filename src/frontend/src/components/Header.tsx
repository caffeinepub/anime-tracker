...
import { AlertCircle, Home, List, LogIn, LogOut } from "lucide-react";
import { useEffect, useState } from "react";
import { useInternetIdentity } from "../hooks/useInternetIdentity";
import {
  useCheckLoginLockout,
  useGetRemainingLockoutTime,
  useRecordFailedLoginAttempt,
  useResetLoginAttempts,
...
export default function Header({ currentPage, onNavigate }: HeaderProps) {
  ...
  const { data: remainingLockoutTime = BigInt(0) } =
    useGetRemainingLockoutTime();
  ...
  const [previousLoginStatus, setPreviousLoginStatus] =
    useState<string>(loginStatus);
