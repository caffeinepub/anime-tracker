...
export default function AnimeGrid({
  ...
  const [currentTitles, setCurrentTitles] = useState<Map<number, number>>(
    new Map(),
  );
    ...
    <>
      ...
      >
        {animeList.map((anime, index) => {
          ...
          const _isRightmostColumn = index % gridColumns === gridColumns - 1;
...
function WatchlistTooltipPortal({
  ...
  const { data: watchlistNames, isLoading } = useGetWatchlistNamesForAnime(
    BigInt(animeId),
  );
  ...
  const tooltipContent = (
    <div
      ...
      style={{
        ...
        right: position.isRightmost
          ? `${window.innerWidth - position.x}px`
          : "auto",
        top: `${position.y}px`,
          ...
          <>
            ...
            <div className="space-y-1">
              {watchlistNames.map((name) => (
                <p key={name} className="text-xs text-yellow-400">
                  • {name}
                </p>
              ))}
            </div>
