...
export default function WatchlistView({
  ...
  const [moveOrCopyEntry, setMoveOrCopyEntry] = useState<WatchlistEntry | null>(
    null,
  );
  const [editingGenresEntry, setEditingGenresEntry] =
    useState<WatchlistEntry | null>(null);
  const [editingNotesEntry, setEditingNotesEntry] =
    useState<WatchlistEntry | null>(null);
  ...
  const [editingRatingEntry, setEditingRatingEntry] =
    useState<WatchlistEntry | null>(null);
  const [ratingValue, setRatingValue] = useState("");
  const [editingEpisodesEntry, setEditingEpisodesEntry] =
    useState<WatchlistEntry | null>(null);
  const [episodesValue, setEpisodesValue] = useState("");
  const [editingTotalEpisodesEntry, setEditingTotalEpisodesEntry] =
    useState<WatchlistEntry | null>(null);
  ...
  const handleNotesHover = (entry: WatchlistEntry, _e: React.MouseEvent) => {
  ...
  const handleRatingClick = (entry: WatchlistEntry, e: React.MouseEvent) => {
    ...
    setRatingValue(
      entry.personalRating > 0 ? entry.personalRating.toString() : "",
    );
  };
  ...
  const handleEpisodesClick = (entry: WatchlistEntry, e: React.MouseEvent) => {
    ...
    setEpisodesValue(
      entry.episodesWatched > 0
        ? Number(entry.episodesWatched).toString()
        : "0",
    );
  };
  ...
  const handleEpisodesSave = async () => {
    ...
    if (episodes > Number(editingEpisodesEntry.anime.episodesAvailable)) {
      toast.error(
        `Episodes watched cannot exceed ${editingEpisodesEntry.anime.episodesAvailable}`,
      );
      return;
    }
  ...
  const handleTotalEpisodesClick = (
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ scrollBehavior: "smooth", marginLeft: "calc(-50vw + 50%)" }}
      >
        <div
                      ...
                      <div className="flex min-h-[110px] items-center gap-2 py-2">
                        ...
                        >
                          <div className="grid grid-cols-2 gap-1">
                            {entry.anime.genres.slice(0, 8).map((genre) => (
                              <div
                                key={genre}
                        ...
                        <div className="flex-shrink-0 flex items-center gap-1 pr-2 ml-auto self-center">
                          ...
                          <button
                            type="button"
                            onClick={(e) =>
                              handleToggleBookmark(entry.anime.anilistId, e)
                            }
      ...
      {/* Notes Editor Modal */}
      <Dialog
        open={!!editingNotesEntry}
        onOpenChange={(open) => !open && handleNotesCancel()}
      >
        <DialogContent className="bg-black border-4 border-yellow-500 text-white max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-yellow-500 text-xl">
              {editingNotesEntry
                ? `Edit Notes - ${getCurrentTitle(editingNotesEntry)}`
                : "Edit Notes"}
            </DialogTitle>
          </DialogHeader>
      ...
      {/* Rating Editor Modal */}
      <Dialog
        open={!!editingRatingEntry}
        onOpenChange={(open) => !open && handleRatingCancel()}
      >
        <DialogContent className="bg-black border-4 border-yellow-500 text-white max-w-md">
          <DialogHeader>
            <DialogTitle className="text-yellow-500 text-xl">
              {editingRatingEntry
                ? `Rate - ${getCurrentTitle(editingRatingEntry)}`
                : "Rate Anime"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <label htmlFor="rating-input" className="text-sm text-gray-400">
      ...
      <Dialog
        open={!!editingEpisodesEntry}
        onOpenChange={(open) => !open && handleEpisodesCancel()}
      >
        <DialogContent className="bg-black border-4 border-yellow-500 text-white max-w-md">
          <DialogHeader>
            <DialogTitle className="text-yellow-500 text-xl">
              {editingEpisodesEntry
                ? `Episodes Watched - ${getCurrentTitle(editingEpisodesEntry)}`
                : "Episodes Watched"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <label
                htmlFor="episodes-watched-input"
                className="text-sm text-gray-400"
              >
                Enter episodes watched (0-
                {editingEpisodesEntry
                  ? Number(editingEpisodesEntry.anime.episodesAvailable)
                  : 0}
                ):
              </label>
              <Input
                id="episodes-watched-input"
                type="number"
                min="0"
                max={
                  editingEpisodesEntry
      ...
      <Dialog
        open={!!editingTotalEpisodesEntry}
        onOpenChange={(open) => !open && handleTotalEpisodesCancel()}
      >
        <DialogContent className="bg-black border-4 border-yellow-500 text-white max-w-md">
          <DialogHeader>
            <DialogTitle className="text-yellow-500 text-xl">
              {editingTotalEpisodesEntry
                ? `Total Episodes - ${getCurrentTitle(editingTotalEpisodesEntry)}`
                : "Total Episodes"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                This will update total episodes across all watchlists containing
                this anime.
              </p>
            </div>
          </div>
